...
export default function VideosAnunciantes() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      try {
        const filePath = await uploadFile(file, "videos-anunciantes");
        const maxOrder =
          videos.length > 0
            ? Math.max(...videos.map((v) => v.sort_order))
            : 0;

        await supabase.from("advertiser_videos").insert({
  ...
  const handleDelete = async (video: Video) => {
    ...
    const supabase = createClient();
    await supabase.from("advertiser_videos").delete().eq("id", video.id);
    try {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">
            Vídeos exibidos no carrossel da seção Anuncie
          </p>
        </div>
